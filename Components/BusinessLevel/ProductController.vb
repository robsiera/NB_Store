...
#Region "NB_Store_Products Public Methods"
        ...
        Public Function GetProductExportList(ByVal PortalID As Integer, ByVal Lang As String, ByVal DeletedOnly As Boolean) As ArrayList
            Return GetProductExportList(PortalID, Lang, DeletedOnly, "")
        End Function

        Public Function GetProductExportList(ByVal PortalID As Integer, ByVal Lang As String, ByVal DeletedOnly As Boolean, ByVal CategoryList As String) As ArrayList
            Return CBO.FillCollection(DataProvider.Instance().GetProductExportList(PortalID, Lang, DeletedOnly, CategoryList), GetType(NB_Store_ProductsInfo))
        End Function
...
#Region "NB_Store_Model Public Methods"
        ...
        Public Function GetModelByProductID(ByVal ProductID As Integer, ByVal Lang As String) As NB_Store_ModelInfo
            Return CType(CBO.FillObject(DataProvider.Instance().GetNB_Store_ModelByProductID(ProductID, Lang), GetType(NB_Store_ModelInfo)), NB_Store_ModelInfo)
        End Function
...
#Region "General Functions"
        ...
        Public Function IsInCategoryOrSubCategory(ByVal PortalID As Integer, ByVal ProductID As Integer, ByVal CategoryID As String) As Boolean
            Dim rtnBool As Boolean = False
            Dim strCacheKey As String = ""

            strCacheKey = GetCacheKey("IsInCategoryOrSubCategory" & CategoryID, PortalID, ProductID)

            If (DataCache.GetCache(strCacheKey) Is Nothing And strCacheKey <> "") Or GetStoreSettingBoolean(PortalID, "debug.mode", "None") Then
                Dim CatList As Hashtable
                Dim aryList As ArrayList
                CatList = getSubCategoryHashTable(PortalID, CategoryID)
                aryList = GetCategoriesAssigned(ProductID)
                For Each objC As NB_Store_ProductCategoryInfo In aryList
                    If CatList.ContainsKey(objC.CategoryID.ToString) Then
                        rtnBool = True
                        DataCache.SetCache(strCacheKey, rtnBool.ToString, DateAdd(DateInterval.Day, 1, Now))
                        Exit For
                    End If
                Next
            Else
                rtnBool = CBool(DataCache.GetCache(strCacheKey))
            End If

            Return rtnBool
        End Function

        Public Function IsInCategory(ByVal PortalID As Integer, ByVal ProductID As Integer, ByVal CategoryID As String) As Boolean
            Dim rtnBool As Boolean = False
            Dim strCacheKey As String = ""

            strCacheKey = GetCacheKey("IsInCategory" & CategoryID, PortalID, ProductID)
...
#Region "Private functions"
        ...
        Private Function getSubCategoryHashTable(ByVal PortalID As Integer, ByVal ParentCategoryID As String) As Hashtable
            Dim CatList As Hashtable

            Dim CacheKey As String = "SubCategoryHashTable_" & PortalID.ToString & "_" & ParentCategoryID.ToString

            If DataCache.GetCache(CacheKey) Is Nothing Or GetStoreSettingBoolean(PortalID, "debug.mode") Then
                Dim aryList As ArrayList
                Dim rtnBool As Boolean = False
                Dim objCCtrl As New CategoryController
                ...
                CatList = New Hashtable
                aryList = objCCtrl.GetCategories(PortalID, GetCurrentCulture)
                CategoryList = objCCtrl.GetSubCategoryList(aryList, ParentCategoryID)
                CategoryList &= ParentCategoryID.ToString

                strCatList = CategoryList.Split(","c)

                For lp As Integer = 0 To strCatList.GetUpperBound(0)
                    CatList.Add(strCatList(lp), strCatList(lp))
                Next

                DataCache.SetCache(CacheKey, CatList, DateAdd(DateInterval.Day, 1, Now))

            Else
                CatList = DataCache.GetCache(CacheKey)
            End If
