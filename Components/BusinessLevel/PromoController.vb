...
#Region "get Coupon Level Methods"
      ...
      Public Function getCouponLevelDiscount(ByVal PortalID As Integer, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo, ByVal CouponCode As String) As Decimal
         ...
         For Each objPromoInfo As NB_Store_PromoInfo In aryList
            ApplyDiscount = True
            'check qty range
            ' Changed by....: Michael Tobisch, helferlein.com
            ' Reason........: Implemented function to check quantity range because I needed that on another place as well (see below)
            '                 This also allows entering "0" in the max. quantity field, which means it is infinite
            If IsInQuantityRange(tQty, objPromoInfo.QtyRange1, objPromoInfo.QtyRange2) = False Then
               ApplyDiscount = False
            End If
            'check price range
            ' Changed by....: Michael Tobisch, helferlein.com
            ' Reason........: Implemented function to check price range because I needed that on another place as well (see below)
            '                 This also allows entering "0" in the max. price field, which means it is infinite
            If IsInPriceRange(tPrice, objPromoInfo.Range1, objPromoInfo.Range2) = False Then
               ApplyDiscount = False
            End If
            'check user roles
            If Not UserInfo.IsInRole(objPromoInfo.PromoGroup) And objPromoInfo.PromoGroup <> "" Then
            ...
            If (objPromoInfo.PromoCode = CouponCode) And ApplyDiscount Then
               ApplyDiscount = True
            Else
               ApplyDiscount = False
            End If
      ...
      Public Function getCouponCartItems(ByVal PortalID As Integer, ByVal CouponCode As String, ByVal IsInCategory As Boolean) As ArrayList
         ...
         For Each objPromoInfo As NB_Store_PromoInfo In aryList
            ...
            If objPromoInfo.PromoCode = CouponCode Then
               ...
               ' Changed by....: Michael Tobisch, helferlein.com
               ' Reason........: This allows to have multiple Discount Coupons with the same Coupon Code but differences in price or quantity range
               '                 (Maybe it would be perfect to add role, email, user, group etc. checks as well, but that is not what I needed :-))
               For Each objInfo In aryCList
                  objMInfo = objPCtrl.GetModel(objInfo.ModelID, GetCurrentCulture)
                  If objPCtrl.IsInCategory(objMInfo.PortalID, objMInfo.ProductID, objPromoInfo.ObjectID) = IsInCategory Or objPromoInfo.ObjectID = -1 Then
                     If (IsInQuantityRange(objInfo.Quantity, objPromoInfo.QtyRange1, objPromoInfo.QtyRange2) = True) AndAlso _
                        IsInPriceRange(objInfo.Quantity * objInfo.UnitCost, objPromoInfo.Range1, objPromoInfo.Range2) = True Then
      ...
      Private Function IsInQuantityRange(ByVal QtyToCheck As Integer, ByVal QtyMin As Integer, ByVal QtyMax As Integer) As Boolean
         ' Added by......: Michael Tobisch, helferlein.com
         ' Description...: Checks if the quantity is in the coupon range of quantities
         ' Parameters....: +--------------+---------+--------------------------------------------------------+
         '                 | Name         | Type    | Description                                            |
         '                 +--------------+---------+--------------------------------------------------------+
         '                 | QtyToCheck   | Integer | The quantity of items in the cart                      |
         '                 | QtyMin       | Integer | The minimum quantity to take the coupon into effect    |
         '                 | QtyMax       | Integer | The maximum quantity to take the coupon into effect    |
         '                 +--------------+---------+--------------------------------------------------------+
         ' Returns.......: Boolean
         ' Remarks.......: - If QtyMin is 0, there is no minimum quantity
         '                 - If QtyMax is 0, there is no maximum quantity, means QtyMax is infinite
         If QtyMin = 0 AndAlso QtyMax = 0 Then
            Return True
         Else
            If QtyMin = 0 AndAlso QtyToCheck <= QtyMax Then
               Return True
            Else
               If QtyMax = 0 AndAlso QtyToCheck >= QtyMin Then
                  Return True
               Else
                  If QtyToCheck >= QtyMin AndAlso QtyToCheck <= QtyMax Then
                     Return True
                  Else
                     Return False
                  End If
      ...
      Private Function IsInPriceRange(ByVal PriceToCheck As Decimal, ByVal PriceMin As Decimal, ByVal PriceMax As Decimal) As Boolean
         ' Added by......: Michael Tobisch, helferlein.com
         ' Description...: Checks if the price is in the coupon range of prices
         ' Parameters....: +--------------+---------+--------------------------------------------------------+
         '                 | Name         | Type    | Description                                            |
         '                 +--------------+---------+--------------------------------------------------------+
         '                 | PriceToCheck | Decimal | The total price of items in the cart                   |
         '                 | PriceMin     | Decimal | The minimum price to take the coupon into effect       |
         '                 | PriceMax     | Decimal | The maximum price to take the coupon into effect       |
         '                 +--------------+---------+--------------------------------------------------------+
         ' Returns.......: Boolean
         ' Remarks.......: - If PriceMin is 0, there is no minimum price
         '                 - If PriceMax is 0, there is no maximum price, means PriceMax is infinite
         If PriceMin = 0 AndAlso PriceMax = 0 Then
            Return True
         Else
            If PriceMin = 0 AndAlso PriceToCheck <= PriceMax Then
               Return True
            Else
               If PriceMax = 0 AndAlso PriceToCheck >= PriceMin Then
                  Return True
               Else
                  If PriceToCheck >= PriceMin AndAlso PriceToCheck <= PriceMax Then
                     Return True
                  Else
                     Return False
                  End If
