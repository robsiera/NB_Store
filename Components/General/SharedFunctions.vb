...
Namespace NEvoWeb.Modules.NB_Store
   ...
   Public Class SharedFunctions
      ...
      Public Const STOREPREFIX As String = "NB_Store2_"
      ...
      Public Shared Sub SendStoreEmail(ByVal Portalid As Integer, ByVal ClientEmail As String, ByVal SubjectText As String, ByVal objOrderInfo As NB_Store_OrdersInfo, ByVal EmailTemplateName As String, ByVal Lang As String)
         ...
         If (GetStoreSettingBoolean(Portalid, "bcceveryoutgoingmailtothemanger.flag") = True) Then
            BccAddress = StoreEmail
         End If

         DotNetNuke.Services.Mail.Mail.SendMail(StoreEmail, ClientEmail, BccAddress, SubjectText, EmailText, "", "HTML", "", "", "", "")
...
#Region "Order Status and Action"
      ...
      Public Shared Function getActionsList(ByVal Lang As String) As Hashtable
         Dim objOCtrl As New OrderController
         Dim arylist As ArrayList
         Dim objAInfo As NB_Store_OrderActionsInfo
         Dim htab As New Hashtable

         arylist = objOCtrl.GetOrderActionsList(Lang)

         For Each objAInfo In arylist
            htab.Add(objAInfo.OrderActionsID, objAInfo.ActionText)
         Next
         Return htab
      End Function
...
#Region "populate lists"
      ...
      Public Shared Sub populateCategoryTextList(ByVal PortalId As Integer, ByVal ddlCategories As ListBox, ByVal DefaultSelectValue As String, ByVal DefaultSelectText As String, ByVal SelectedValue As String)
         Dim objCtrl As New CategoryController
         Dim li As ListItem
         Dim aryList As ArrayList
         ...
         BuildCategoryTextList(aryList, ddlCategories, 0, "")
      ...
      Public Shared Sub BuildCategoryTextList(ByVal aryList As ArrayList, ByVal ddlCategories As ListBox, ByVal ParentID As Integer, ByVal Prefix As String)
         Dim objCInfo As NB_Store_CategoriesInfo
         Dim li As ListItem
         For Each objCInfo In aryList
            If objCInfo.ParentCategoryID = ParentID Then
               li = New ListItem
               li.Value = objCInfo.CategoryID.ToString
               li.Text = Prefix & objCInfo.ListOrder.ToString & ": " & objCInfo.CategoryName
               ddlCategories.Items.Add(li)
               BuildCategoryTextList(aryList, ddlCategories, objCInfo.CategoryID, Prefix & " . ")
            End If
         Next
      End Sub
...
#End Region
      ...
      Public Shared Function GetCountryCodeByName(ByVal Country As String) As String
         If IsNothing(Country) Then
            Return ""
         End If
         Dim ctlEntry As New Lists.ListController
         Dim entryCollection As Lists.ListEntryInfoCollection = ctlEntry.GetListEntryInfoCollection("Country")
         For Each l As ListEntryInfo In entryCollection
            If (l.Text.ToLower = Country.ToLower) Then
               Return l.Value
            End If
         Next
         Return ""
      ...
      Public Shared Sub CreateDir(ByVal PortalSettings As DotNetNuke.Entities.Portals.PortalSettings, ByVal FolderName As String)
         ...
         If Not blnCreated Then
            FileSystemUtils.SynchronizeFolder(PortalSettings.PortalId, PortalSettings.HomeDirectoryMapPath, PortalSettings.HomeDirectory, True)
      ...
      Public Shared Function GetOptCodeInfo(ByVal PortalID As Integer, ByVal OptCode As String, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo, ByVal TextOptions As String, ByVal TextOptionsXML As String) As OptCodeInfo
         ...
         Dim showOptionNames As Boolean = GetStoreSettingBoolean(PortalID, "showOptionNames.flag", GetCurrentCulture)
            ...
            For lp = 0 To optC.GetUpperBound(0)
               If IsNumeric(optC(lp)) Then
                  ...
                  Else
                     ...
                     objOInfo = objCtrl.GetOption(objOVInfo.OptionID, GetCurrentCulture) 'new line
                     If Not objOVInfo Is Nothing Then
                        rtnAmount.UnitCost = rtnAmount.UnitCost + objOVInfo.AddedCost
                        If showOptionNames Then
                           rtnAmount.ItemDesc &= objOInfo.OptionDesc & " : " & objOVInfo.OptionValueDesc & OptSeperator 'edit add optionDesc
                        Else
      ...
      Public Shared Function IsEditor(ByVal Portalid As Integer, ByVal Usrinfo As Users.UserInfo) As Boolean
         ...
         Else
            ...
            Else
               ...
               Dim eRoles As String() = EditorRole.Split(","c)
               Dim r As String = ""
               For Each r In eRoles
                  If Usrinfo.IsInRole(r) Then
                     Return True
                  End If
               Next
               Return False
...
#Region "Store Settings Functions"
...
#End Region
      ...
      Public Shared Function populateCategoryCheckBoxList(ByVal PortalId As Integer, ByVal EntryID As Integer, ByVal chkCategories As CheckBoxList) As Integer
         ...
         Else
            ...
            Return chkCategories.Items.Count
         End If
      ...
      Public Shared Function GetAvailableModelList(ByVal PortalID As Integer, ByVal ProductID As Integer, ByVal Lang As String, ByVal IsDealer As Boolean) As ArrayList
         ...
         If GetStoreSettingBoolean(PortalID, "outofstockmodels.flag") Then
            ...
            For lp As Integer = (aryStock.Count - 1) To 0 Step -1
               ...
               End If
               'remove all models with dealercost < 0
               If ((CType(aryStock.Item(lp), NB_Store_ModelInfo).DealerCost < 0) And IsDealer) Then
                  aryStock.RemoveAt(lp)
               End If
            Next
         Else
...
#Region "Javascript methods"
      ...
      Public Shared Sub IncludeScripts(ByVal PortalID As Integer, ByVal StoreInstallPath As String, ByVal Page As System.Web.UI.Page, ByVal JSIncludeKey As String, ByVal JSStartUpIncludeKey As String, ByVal CSSIncludeKey As String)
         ...
         'DNN 5 use DNN version (DNN4 no longer supported)
         Try
         ...
         If Not objSInfo Is Nothing Then
            ...
            If objSInfo.SettingValue.Trim <> "" Then
               ...
               For lp As Integer = 0 To jsList.GetUpperBound(0)
                  ...
                  ' fix to remove unrequire js from DNN6 (need to leave in DNN5 for the time being.)
                  If DotNetNuke.Application.DotNetNukeContext.Current.Application.Version.Major >= 6 Then
                     If jsList(lp) = "jquery.dimensions.js" Then jsList(lp) = ""
                  End If
...
#End Region
      ...
      Public Shared Sub removeStoreCookie(ByVal PortalID As Integer, ByVal CookieName As String)
         Dim FullCookieName As String = STOREPREFIX & CookieName & "_Portal" & PortalID.ToString
      ...
      Public Shared Function getStoreCookieValue(ByVal PortalID As Integer, ByVal objCookie As HttpCookie, ByVal ValueID As String) As String
      ...
      Public Shared Function getStoreCookieValue(ByVal PortalID As Integer, ByVal CookieName As String, ByVal ValueID As String) As String
         Dim FullCookieName As String = STOREPREFIX & CookieName & "_Portal" & PortalID.ToString
         Dim FoundCookie As HttpCookie = HttpContext.Current.Request.Cookies(FullCookieName)
         Return getStoreCookieValue(PortalID, FoundCookie, ValueID)
      End Function
      ...
      Public Shared Sub setStoreCookieValue(ByVal PortalID As Integer, ByVal objCookie As HttpCookie, ByVal ValueID As String, ByVal Value As String, Optional ByVal ExpireDays As Double = 30)
         ...
         If Not objCookie Is Nothing Then
            ...
            If ExpireDays = "0" Then
               objCookie.Expires = Nothing
            Else
               objCookie.Expires = DateAdd(DateInterval.Day, ExpireDays, Today)
            End If
      ...
      Public Shared Sub setStoreCookieValue(ByVal PortalID As Integer, ByVal CookieName As String, ByVal ValueID As String, ByVal Value As String, Optional ByVal ExpireDays As Double = 30)
         ...
         If FoundCookie Is Nothing Then
            FoundCookie = New HttpCookie(FullCookieName)
         End If
         setStoreCookieValue(PortalID, FoundCookie, ValueID, Value, ExpireDays)

      End Sub

      Public Shared Function StoreEncrypt(ByVal PortalID As Integer, ByVal Value As String) As String
         Dim objSec As New PortalSecurity
         Dim encryptKey As String = GetStoreSetting(PortalID, "encrypt.key")
         ...
         Else
            If Value Is Nothing Then
               Return ""
            Else
               Return objSec.Encrypt(encryptKey, Value)
            End If
         End If
      End Function

      Public Shared Function StoreDecrypt(ByVal PortalID As Integer, ByVal Value As String) As String
         Dim objSec As New PortalSecurity
         Dim encryptKey As String = GetStoreSetting(PortalID, "encrypt.key")
         ...
         Else
            If Value Is Nothing Then
               Return ""
            Else
               Return objSec.Decrypt(encryptKey, Value)
            End If
         End If
      End Function
...
#Region "Query String Cookies"

      Public Shared Function getCookieQueryValueInt(ByVal PortalID As Integer, ByVal CookieValueName As String, ByVal ListCookie As HttpCookie, ByVal Request As HttpRequest, ByVal DefaultReturn As Integer) As Integer
         Dim rtnStr As String = getCookieQueryValue(PortalID, CookieValueName, ListCookie, Request)
         If IsNumeric(rtnStr) Then
            Return CInt(rtnStr)
         Else
      ...
      Public Shared Function getCookieQueryValue(ByVal PortalID As Integer, ByVal CookieValueName As String, ByVal ListCookie As HttpCookie, ByVal Request As HttpRequest) As String
         Return getCookieQueryValue(PortalID, CookieValueName, ListCookie, Request, "")
      End Function

      Public Shared Function getCookieQueryValue(ByVal PortalID As Integer, ByVal CookieValueName As String, ByVal ListCookie As HttpCookie, ByVal Request As HttpRequest, ByVal DefaultReturn As String) As String
         Dim rtnValue As String = ""
         If Not (Request.QueryString(CookieValueName) Is Nothing) Then
            rtnValue = Request.QueryString(CookieValueName)
            setStoreCookieValue(PortalID, ListCookie, CookieValueName, rtnValue.ToString, 0)
         Else
            rtnValue = getStoreCookieValue(PortalID, ListCookie, CookieValueName)
         End If
         If rtnValue = "" Then rtnValue = DefaultReturn
         Return rtnValue
      End Function

      Public Shared Sub removeCookieQueryValue(ByVal PortalID As Integer, ByVal CookieValueName As String, ByVal ListCookie As HttpCookie)
         setStoreCookieValue(PortalID, ListCookie, CookieValueName, "", 0)
      End Sub
...
#Region " Urls "
      ...
      Public Shared Function getUrlCookieInfo(ByVal PortalID As Integer, ByVal ParamArray AdditionalParameters() As String) As String()
         ...
         Dim params(AdditionalParameters.Length + 8) As String
         ...
         params(7) = getCookieURLparam(PortalID, "desc")

         For i As Integer = 0 To AdditionalParameters.Length - 1
            params(i + 9) = AdditionalParameters(i)
         Next
      ...
      Public Shared Sub IncludeCanonicalLink(ByVal page As Page, ByVal href As String)
         If href <> "" Then
            If page.Header.FindControl("nbstorecanonical") Is Nothing Then
               Dim cLink As HtmlLink
               cLink = New HtmlLink()
               cLink.ID = "nbstorecanonical"
      ...
      Public Shared Function ProductPageName(ByVal ProductListInfo As ProductListInfo, ByVal CategoryName As String, ByVal PortalName As String) As String
         ...
         Dim strText As String = PortalName
         If Not ProductListInfo Is Nothing Then

            Dim CacheKey As String = "PRDPG" & ProductListInfo.ProductID.ToString & "_" & ProductListInfo.Lang

            If DataCache.GetCache(CacheKey) Is Nothing Or GetStoreSettingBoolean(ProductListInfo.PortalID, "debug.mode") Then
               ...
               Else
                  ...
                  End If
               End If
               DataCache.SetCache(CacheKey, strText, DateAdd(DateInterval.Day, 1, Now))
            Else
               strText = DataCache.GetCache(CacheKey)
            End If
      ...
      Public Shared Function CategoryPageName(ByVal CategoryListInfo As NB_Store_CategoriesInfo, ByVal PortalName As String, ByVal PageTitle As String) As String
         Dim strText As String = PortalName
         If Not CategoryListInfo Is Nothing Then

            Dim CacheKey As String = "CATPG" & CategoryListInfo.CategoryID.ToString & "_" & CategoryListInfo.Lang

            If DataCache.GetCache(CacheKey) Is Nothing Or GetStoreSettingBoolean(CategoryListInfo.PortalID, "debug.mode") Then
            ...
            Else
               strText = DataCache.GetCache(CacheKey)
            End If
            If strText = "" Then strText = PortalName
         End If
...
#Region "Order Action Log"

      Public Shared Sub updateOrderActionLog(ByVal objOInfo As NB_Store_OrdersInfo, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo, ByVal ActionsID As Integer)
         Dim objOCtrl As New OrderController
         Dim objAInfo As New NB_Store_OrderActionInfo

         objAInfo.ActionDate = Now
         objAInfo.OrderActionID = -1
         objAInfo.OrderActionsID = ActionsID
         objAInfo.OrderID = objOInfo.OrderID
         If UserInfo Is Nothing Then
            objAInfo.username = ""
         Else
            objAInfo.username = UserInfo.Username
         End If
         objAInfo.OrderStatusID = objOInfo.OrderStatusID

         objOCtrl.UpdateObjOrderAction(objAInfo)
