    ...
    Public Class ProductTemplate
        ...
        Sub InstantiateIn(ByVal container As Control) Implements ITemplate.InstantiateIn
            ...
            For lp = 0 To _aryTempl.GetUpperBound(0)
                If Not _aryTempl(lp) Is Nothing Then
                    ...
                    Select Case _aryTempl(lp).ToUpper
                        ...
                        Case "TAG:FROMPRICETAX"
                            lc = New Literal
                            AddHandler lc.DataBinding, AddressOf FromPriceTax_DataBinding
                            container.Controls.Add(lc)
                        Case "TAG:FROMPRICETAXCURRENCY"
                            lc = New Literal
                            AddHandler lc.DataBinding, AddressOf FromPriceTaxCurrency_DataBinding
                            container.Controls.Add(lc)
                        Case "TAG:TAXPERCENT"
                            lc = New Literal
                            AddHandler lc.DataBinding, AddressOf TaxPercentApplied_DataBinding
                            container.Controls.Add(lc)
                        ...
                        Case "TAG:PRICESALETAXCURRENCY"
                            lc = New Literal
                            AddHandler lc.DataBinding, AddressOf PriceSaleTaxCurrency_DataBinding
                            container.Controls.Add(lc)
                                ...
                                Else
                                    If _aryTempl(lp).ToUpper.StartsWith("TAG:DOC") Then
                                        ...
                                        ElseIf _aryTempl(lp).ToUpper.StartsWith("TAG:DOCDESC") Then
                                            ...
                                            container.Controls.Add(lc)
                                        ElseIf _aryTempl(lp).ToUpper.StartsWith("TAG:DOCNAME") Then
                                            lc = New Literal
                                            lc = assignProperties(_aryTempl(lp), lc)
                                            AddHandler lc.DataBinding, AddressOf DocName_DataBinding
                                            container.Controls.Add(lc)
                                        Else
        ...
        Private Sub Link_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            '-----------------------------------------------------------------------------------------
            ' this Horrible bit of code is here so we can pickup the default product category and hence have the correct product detail template displayed.
            ...
            aryList = objCtrl.GetCategoriesAssigned(DataBinder.Eval(container.DataItem, "ProductID"))
        ...
        Private Sub FromPrice_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub
        Private Sub FromPriceTax_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            Dim lc As Literal
            lc = CType(sender, Literal)
            Dim container As DataListItem
            container = CType(lc.NamingContainer, DataListItem)
            lc.Text = getFromPriceWithAppliedTax(container, False)
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub
        Private Sub FromPriceCurrency_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub
        Private Sub FromPriceTaxCurrency_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            Dim lc As Literal
            lc = CType(sender, Literal)
            Dim container As DataListItem
            container = CType(lc.NamingContainer, DataListItem)
            lc.Text = getFromPriceWithAppliedTax(container, True)
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub

        Private Sub TaxPercentApplied_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            Dim lc As Literal
            lc = CType(sender, Literal)
            Dim container As DataListItem
            container = CType(lc.NamingContainer, DataListItem)
            lc.Text = getTaxPercentApplied(container)
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub
        ...
        Private Sub PriceSaleTaxCurrency_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            Dim lc As Literal
            lc = CType(sender, Literal)
            Dim container As DataListItem
            container = CType(lc.NamingContainer, DataListItem)
            lc.Text = getSalePrice(container, True, True)
            lc.Visible = CBool(_NestedLevel((_NestedLevel.Count - 1)))
        End Sub
        ...
        Private Sub Image_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            Else
                lc.ImageUrl = _StoreInstallPath & "makethumbnail.ashx?Image=" & ImageID.ToString & "&w=" & ThumbW & "&tabid=" & _TabID & "&h=" & ThumbH
            End If
            If DataBinder.Eval(container.DataItem, "ImageDesc") = "" Then
                lc.AlternateText = DataBinder.Eval(container.DataItem, "SEOName")
                If lc.AlternateText = "" Then
                    lc.AlternateText = DataBinder.Eval(container.DataItem, "ProductName")
                End If
            Else
        ...
        Private Sub ImageLB_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            If DataBinder.Eval(container.DataItem, "ImageDesc") = "" Then
                If DataBinder.Eval(container.DataItem, "SEOName") = "" Then
                    strData = Replace(strData, "[Product:ImageDesc]", DataBinder.Eval(container.DataItem, "ProductName"))
                Else
                    strData = Replace(strData, "[Product:ImageDesc]", DataBinder.Eval(container.DataItem, "SEOName"))
                End If
            Else
        ...
        Private Sub DocName_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            Dim lc As Literal
            lc = CType(sender, Literal)
            Dim container As DataListItem
            container = CType(lc.NamingContainer, DataListItem)
            ...
            Dim arylist As ArrayList
            Dim objDInfo As NB_Store_ProductDocInfo
            Dim strButtonType As String = "docname"
            ...
            If DocIndex >= 0 Then
                ...
                If arylist.Count >= (DocIndex + 1) Then
                    objDInfo = CType(arylist(DocIndex), NB_Store_ProductDocInfo)
                    lc.Text = objDInfo.FileName.Replace("%20", " ") ' decode html spaces in filename
                End If
            End If
        ...
        Private Sub AddToRelated_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            'Begin of changes
            Dim strProductID As String = DataBinder.Eval(container.DataItem, "ProductID")
            cmd.Attributes.Add("onclick", String.Format("javascript:return addToRelated(this, '{0}', {1}, {2});", System.Web.VirtualPathUtility.ToAbsolute("~/DesktopModules/NB_Store/XMLProvider.ashx"), System.Web.HttpContext.Current.Request.QueryString("ProdId"), strProductID))
            'End of changes

            cmd.CommandArgument = strProductID
        ...
        Private Sub OptionText_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            If OptionIndex >= 0 Then
                ...
                If aryList.Count >= (OptionIndex + 1) Then
                    txt.Visible = True
                    objOInfo = CType(aryList(OptionIndex), NB_Store_OptionInfo)
                    'txt.ToolTip = objOInfo.OptionDesc
                    txt.Attributes.Add("OptionDesc", objOInfo.OptionDesc)
                    aryList = objCtrl.GetOptionValueListWithInterface(objOInfo.OptionID, objOInfo.Lang)
                    If aryList.Count >= 1 Then
                        Dim OptionValueInfo As NB_Store_OptionValueInfo = CType(aryList(0), NB_Store_OptionValueInfo)
...
#Region "General"
        ...
        Private Function getAmountWithAppliedTax(ByVal PortalID As Integer, ByVal ProductID As Integer, ByVal Amount As Decimal) As Decimal
            Dim objPCtrl As New ProductController
            Dim objTaxCalc As New TaxCalcController(PortalID)
            Dim objTaxInfo As NB_Store_TaxRatesInfo
            Dim objMInfo As NB_Store_ModelInfo
            Dim rtnAmt As Decimal = Amount

            objMInfo = objPCtrl.GetModelByProductID(ProductID, GetCurrentCulture)
            objTaxInfo = objTaxCalc.getTaxRateUsed(PortalID, objMInfo.ModelID)

            If Not objTaxInfo Is Nothing Then
                Select Case objTaxCalc.getTaxOption
                    Case "2" ' gross
                        rtnAmt = (Amount / (100 + objTaxInfo.TaxPercent)) * 100
                    Case "3" ' net
                        rtnAmt = (Amount * (1 + (objTaxInfo.TaxPercent / 100)))
                End Select
            End If
...
#Region "From Price Method"
        ...
        Private Function getTaxPercentApplied(ByVal container As DataListItem) As String
            Dim ProductID As Double = DataBinder.Eval(container.DataItem, "ProductID")
            Dim PS As Entities.Portals.PortalSettings = Entities.Portals.PortalController.GetCurrentPortalSettings
            Dim objPCtrl As New ProductController
            Dim objMInfo As NB_Store_ModelInfo
            Dim objTaxCalc As New TaxCalcController(PS.PortalId)
            Dim objTaxInfo As NB_Store_TaxRatesInfo

            objMInfo = objPCtrl.GetModelByProductID(ProductID, GetCurrentCulture)

            If Not objMInfo Is Nothing Then
                objTaxInfo = objTaxCalc.getTaxRateUsed(PS.PortalId, objMInfo.ModelID)
                If Not objTaxInfo Is Nothing Then
                    Return objTaxInfo.TaxPercent.ToString()
                Else
                    Return ""
                End If
            Else
        ...
        Private Function getFromPriceWithAppliedTax(ByVal container As DataListItem, ByVal FormatAsCurrency As Boolean) As String
            Dim fromPrice As Double = DataBinder.Eval(container.DataItem, "FromPrice")
            If fromPrice > 0 Then
                Dim ProductID As Double = DataBinder.Eval(container.DataItem, "ProductID")
                Dim PS As Entities.Portals.PortalSettings = Entities.Portals.PortalController.GetCurrentPortalSettings

                fromPrice = getAmountWithAppliedTax(PS.PortalId, ProductID, fromPrice)
...
#Region "Sale Price Method"
        ...
        Private Function getSalePrice(ByVal container As DataListItem, ByVal FormatAsCurrency As Boolean) As String
            Return getSalePrice(container, FormatAsCurrency, False)
        End Function

        Private Function getSalePrice(ByVal container As DataListItem, ByVal FormatAsCurrency As Boolean, ByVal ApplyTax As Boolean) As String
            Dim rtnStr As String
            Dim PS As Entities.Portals.PortalSettings = Entities.Portals.PortalController.GetCurrentPortalSettings
            Dim strCacheKey As String = ""
            ...
            If (DataCache.GetCache(strCacheKey) Is Nothing And strCacheKey <> "") Or GetStoreSettingBoolean(PS.PortalId, "debug.mode", "None") Or _UserInfo.UserID >= 0 Then
                ...
                Else
                    If ApplyTax Then
                        SalePrice = getAmountWithAppliedTax(PS.PortalId, ProdID, SalePrice)
                    End If
        ...
        Private Function getFromDealerPrice(ByVal container As DataListItem, ByVal FormatAsCurrency As Boolean) As String
            ...
            If (DataCache.GetCache(strCacheKey) Is Nothing And strCacheKey <> "") Or GetStoreSettingBoolean(PS.PortalId, "debug.mode", "None") Then
                ...
                For Each objMInfo As NB_Store_ModelInfo In aryList
                    ...
                    If (objMInfo.DealerCost < rtnPrice) And (objMInfo.DealerCost > 0) Then
