...
#Region "Cart Public Methods"
      ...
      Public Shared Function CreateOrder(ByVal UserID As Integer, ByVal PortalID As Integer, ByVal NoteMsg As String, ByVal OrderEmail As String, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo) As NB_Store_OrdersInfo
         ...
         objInfo.OrderStatusID = Constants.OrderStatus.Incomplete
         objInfo.PayType = ""
         ...
         objInfo.Referrer = objCInfo.Referrer

         objInfo = CreateOrderTotals(objInfo, objCInfo)

         objInfo = objOCtrl.UpdateObjOrder(objInfo)

         updateOrderActionLog(objInfo, UserInfo, 10) ' create order
      ...
      Public Shared Function GetCartItemTotal(ByVal PortalID As Integer, Optional ByVal lessDiscount As Boolean = False) As Decimal
         Dim aryList As ArrayList
         ...
         For Each objInfo In aryList
            ...
            If lessDiscount Then
               TotalAmt += objInfo.Quantity + objInfo.Discount
            End If
         Next
      ...
      Public Shared Function GetCurrentCart(ByVal PortalID As Integer) As NB_Store_CartInfo
         Return GetCurrentCart(PortalID, Nothing)
      End Function


      Public Shared Function GetCurrentCart(ByVal PortalID As Integer, ByVal Request As HttpRequest) As NB_Store_CartInfo
         Dim objCtrl As New CartController
         Dim objCInfo As NB_Store_CartInfo
         Dim CartID As String
         ...
         If objCInfo Is Nothing Then
            ...
            If Not Request Is Nothing Then
               If Not Request.UrlReferrer Is Nothing Then
                  objCInfo.Referrer = Request.UrlReferrer.AbsoluteUri
               End If
            End If
            objCtrl.UpdateObjCart(objCInfo)
         Else
            If objCInfo.Referrer = "" Then
               If Not Request Is Nothing Then
                  If Not Request.UrlReferrer Is Nothing Then
                     objCInfo.Referrer = Request.UrlReferrer.AbsoluteUri
                     objCtrl.UpdateObjCart(objCInfo)
      ...
      Public Shared Sub AddItemToCart(ByVal PortalID As Integer, ByVal LItem As DataListItem, ByVal IncrementCart As Boolean, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo, Optional ByVal Request As HttpRequest = Nothing)
         ...
         Dim objCIInfo As NB_Store_CartItemsInfo
         Dim objCtrl As New ProductController 'new line added
         Dim objOInfo As New NB_Store_OptionInfo 'new line added
         Dim Ctrl As Control
         ...
         If Not Ctrl Is Nothing Then
            If IsNumeric(DirectCast(Ctrl, HiddenField).Value) Then
               ...
               ' Fix for issue with dealer price being hidden with -1 dealer cost.
               If (IsDealer(PortalID, UserInfo, GetCurrentCulture)) Then
                  ' check if we have multiple models that are hidden from dealer.
                  Dim m As NB_Store_ModelInfo
                  m = objCtrl.GetModel(modelID, GetCurrentCulture)
                  Dim aryList As ArrayList
                  Dim rtnPrice As Decimal = -1
                  If m.DealerCost > 0 Then rtnPrice = m.DealerCost
                  aryList = GetAvailableModelList(PortalID, m.ProductID, GetCurrentCulture, IsDealer(PortalID, UserInfo, GetCurrentCulture))
         ...
         For Each txt In txtOptionCtrls
            If showOptionNames Then
               txtOptionDesc = CType(txt, TextBox).Attributes.Item("OptionDesc") & " : "
            Else
               txtOptionDesc = ""
            End If
            strXML &= "<" & txt.ID.ToLower & ">" & txtOptionDesc & txt.Text & "</" & txt.ID.ToLower & ">" 'new add OptName
            If CType(txt, TextBox).ToolTip = "" Then
               txtOption &= txtOptionDesc & CType(txt, TextBox).Text & optSep 'new add OptName
            Else
               txtOption &= txtOptionDesc & CType(txt, TextBox).ToolTip & "=" & CType(txt, TextBox).Text & optSep 'new add OptName
            End If
         Next
      ...
      Public Shared Sub CancelOrder(ByVal PortalID As Integer)
         ...
         If Not objOInfo Is Nothing Then
            ...
            Else
               ...
               objOInfo.OrderStatusID = Constants.OrderStatus.Cancelled
      ...
      Public Shared Sub ValidateCart(ByVal PortalID As Integer, ByVal UserInfo As DotNetNuke.Entities.Users.UserInfo)
                  ...
                  If blnFound Then
                     ...
                     If StockQty > 0 Then
                        ...
                        If OptCInfo.OptCode = "" Then
                           RemoveItemFromCart(objCInfo.ItemID)
                        Else
                           If Not objCInfo.StaticAmts Then ' cart item amount already adjusted, don't change.
         ...
         ' Description...: If the coupon code is empty and there are default coupon codes for dealers and/or non-dealers, apply these coupon codes
         '                 The default values are defined in the Backoffice > Utilities > Settings section
         '                    Group....: Pricing
         '                    Name.....: dealerdefaultcouponcode.text and nondealerdefaultcouponcode.text
         '                    AdminOnly: false
         '                    Type.....: Textbox
         '                    The values of these settings must then be entered as Code in a Coupon Promotion to take effect
         If String.IsNullOrEmpty(objCartInfo.PromoCode) Then
            If IsDealer(PortalID, UserInfo, GetCurrentCulture) = True Then
               objCartInfo.PromoCode = GetStoreSetting(PortalID, "dealerdefaultcouponcode.text", GetCurrentCulture)
            Else
               objCartInfo.PromoCode = GetStoreSetting(PortalID, "nondealerdefaultcouponcode.text", GetCurrentCulture)
            End If
         End If
