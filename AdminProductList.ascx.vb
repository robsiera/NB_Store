...
#Region "Private Members"
        ...
        'Begin of changes
        Protected _RelatedList As List(Of Integer) = New List(Of Integer)
        'End of changes
        ...
        Private Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
            ...
            If Not (Request.QueryString("rel") Is Nothing) Then
                If Request.QueryString("rel") = "1" Then
                    ...
                    '1 July 2013 by Sergey Velichko: Added functionality to handle ajax calls for add related products
                    'Begin of changes
                    Dim objProductController As ProductController = New ProductController()
                    Dim SubRelatedList As ArrayList = objProductController.GetProductRelatedArray(DotNetNuke.Entities.Portals.PortalController.GetCurrentPortalSettings().PortalId, ProdID, SharedFunctions.GetCurrentCulture(), -1, False)
                    For Each objRelated As NB_Store_ProductRelatedInfo In SubRelatedList
                        _RelatedList.Add(objRelated.RelatedProductID)
                    Next

                    'Add image loader
                    Dim ResourceFile As String = Services.Localization.Localization.GetResourceFile(Me, Me.GetType().BaseType.Name + ".ascx")
                    Dim AjaxLoader As String = String.Format(Localization.GetString("AjaxLoader", ResourceFile), ResolveUrl("img/ajaxLoader.gif"))
                    If (Not Page.ClientScript.IsClientScriptBlockRegistered("ajaxLoader")) Then
                        Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "ajaxLoader", String.Format("<script type='text/javascript'>var ajaxLoader = '{0}';</script>", AjaxLoader))
                    End If
            ...
            If strI <> "" And strA <> "" Then
                dlProducts.ItemTemplate = New ProductTemplate(_Parent.TabId, _Parent.ModuleId, DotNetNuke.Common.ResolveUrl(_Parent.AppRelativeTemplateSourceDirectory), ThumbSize, Server.HtmlDecode(strI), True, "", "", _CurrentPage, _CatID, 100, "", _Parent.TabId, _Parent.UserId, _Parent.UserInfo, "", "", False, 999999, _RelatedList)
                dlProducts.AlternatingItemTemplate = New ProductTemplate(_Parent.TabId, _Parent.ModuleId, DotNetNuke.Common.ResolveUrl(_Parent.AppRelativeTemplateSourceDirectory), ThumbSize, Server.HtmlDecode(strA), True, "", "", _CurrentPage, _CatID, 100, "", _Parent.TabId, _Parent.UserId, _Parent.UserInfo, "", "", False, 999999, _RelatedList)
            End If
