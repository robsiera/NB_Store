...
#Region "Methods"
        ...
        Public Sub PopulateCartList(ByVal OrderID As Integer)
            ...
            If Not aryList Is Nothing Then
                'Add link to product
                AddLinktoDescription(aryList, GetStoreSettingBoolean(PortalID, "cartthumbs.flag", "NONE"))
        ...
        Private Function AddLinktoDescription(ByVal aryList As ArrayList, ByVal AddThumb As Boolean) As ArrayList
            Dim d As String = ""
            For Each a As NB_Store_CartListInfo In aryList
                d = ""
                If AddThumb Then
                    Dim objPCtrl As New ProductController
                    Dim objPIInfo As NB_Store_ProductImageInfo
                    Dim aryImgList As ArrayList

                    Dim m As NB_Store_ModelInfo = objPCtrl.GetModel(a.ModelID, GetCurrentCulture)
                    If Not m Is Nothing Then
                        aryImgList = objPCtrl.GetProductImageList(m.ProductID, GetCurrentCulture)
                        If aryImgList.Count > 0 Then
                            objPIInfo = CType(aryImgList(0), NB_Store_ProductImageInfo)
                            If Not objPIInfo Is Nothing Then
                                Dim thumbSize As Integer = GetStoreSettingInt(PortalID, "cartthumb.size", "NONE")
                                If thumbSize < 40 Then thumbSize = 50
                                d = d & "<img class='cartthumb' src='/DesktopModules/NB_Store/makethumbnail.ashx?Image=" & objPIInfo.ImageID & "&amp;w=" & thumbSize & "&amp;h=0' border='0' />"
                            End If
                        End If
                    End If

                End If
                d = d + "<a href='" & a.ProductURL & "' >" & a.ItemDesc & "</a>"
                a.ItemDesc = d
            Next
            Return aryList
        End Function
