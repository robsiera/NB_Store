 ...
 jQuery(document).ready(function(){
    ...
    jQuery('.textexp_extra').focus(function() {
        if (jQuery(this).width() <= '200') {
            ...
            jQuery('.textexp_extra').animate({ 'width': '230px' }, 'slow');
            return false;
        }
    });
    jQuery('.textexp_name').focus(function() {
        if (jQuery(this).width() <= '200') {
            ...
            jQuery('.textexp_name').animate({ 'width': '230px' }, 'slow');
            return false;
        }
    });
    jQuery('.textexp_code').focus(function() {
        if (jQuery(this).width() <= '200') {
            jQuery('.textexp_extra').animate({ 'width': '40px' }, 'slow');
            jQuery('.textexp_name').animate({ 'width': '40px' }, 'slow');
            jQuery('.textexp_code').animate({ 'width': '230px' }, 'slow');
            return false;
        }
    });
...
//Begin of changes
 function addToRelated(objThis, url, prodID, relID) {
     jQuery(objThis).html(ajaxLoader);
     jQuery.ajax({
         type: "GET",
         async: true,
         url: url + "?action=addToRelated&prodID=" + prodID + "&relID=" + relID,
