...
<asp:label id="lblBrowse" runat="server" Visible="false" resourcekey="lblBrowse">Label</asp:label><asp:FileUpload ID="cmdBrowse" runat="server" />&nbsp;&nbsp;&nbsp;
 <asp:linkbutton id="cmdAddImage" runat="server" cssclass="dnnSecondaryAction NBright_AddButton" Resourcekey="Add">Add</asp:linkbutton>
</div>
<br />
...
<asp:datagrid id="dgImages" runat="server" AutoGenerateColumns="False" CellPadding="1" Width="100%" GridLines="None">
		...
		<Columns>
			...
			<asp:TemplateColumn HeaderText="Hidden" Visible="false">
				<ItemTemplate>
					<asp:CheckBox CssClass="normalCheckBox" id="chkHide" runat="server" Checked='<%# DataBinder.Eval(Container, "DataItem.Hidden") %>'></asp:CheckBox>
				</ItemTemplate>
			</asp:TemplateColumn>
