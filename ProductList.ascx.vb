...
#Region "Private Members"
        ...
        Private _categoryList As String = ""
...
#Region "Event Handlers"
        ...
        Private Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
            ...
            If CType(Settings("chkBrowseCategory"), Boolean) = True Then
                ...
                Else
                    CatID = assignCatDefault()
                    ' no catid in url, so look for a templated version of the product template
                    Dim templCatId As Integer
                    templCatId = GetTemplProductCatID(ProdID)
                    If (ProdID >= 0 And templCatId > 0) Then
                        CatID = templCatId
                    End If
                End If
            Else
            ...
            If Not StaticTemplates Then

                Dim objCCtrl As New CategoryController
                Dim objCInfo As NB_Store_CategoriesInfo

                'check if a template exists for category
                If CatID < 1 Then ' if no category is specified then CatID is -1
                    Dim objPCtrl As New ProductController
                    Dim arrCats As ArrayList
                    arrCats = objPCtrl.GetCategoriesAssigned(ProdID)
                    If Not arrCats Is Nothing Then
                        For Each objPCInfo As NB_Store_ProductCategoryInfo In arrCats
                            If strDTempl = "" Then
                                objCInfo = objCCtrl.GetCategory(objPCInfo.CategoryID, GetCurrentCulture)
                                If objCInfo.ProductTemplate <> "" And Not objCInfo.Archived Then
                                    strDTempl = objCInfo.ProductTemplate
                                    Exit For ' detail template is set so no need to iterate through remaining categories
                                End If
                            End If
                        Next
                    End If
            ...
            Try
                ...
                If Not Request.Params("Search") Is Nothing Then
                    _searchQuery = Server.UrlDecode(Request.Params("Search")).ToString
                End If
                ...
                If Not Page.IsPostBack Then
                    ...
                    'Track Referrer
                    If GetStoreSettingBoolean(PortalId, "Referrertrace.flag") Then
                        CurrentCart.GetCurrentCart(PortalId, Request)
                    End If
...
#Region "Methods"
        ...
        Private Sub populateList(ByVal CatID As Integer)
            ...
            Dim cookiePList As System.Web.HttpCookie = getStoreCookie(PortalId, ModuleId.ToString & "_ProductList")
            ...
            If AllowCategoryBrowse Then
                PgSize = getCookieQueryValueInt(PortalId, "psize", cookiePList, Request, PgSize)
            End If
            ...
            'check if we have category filter (usually created by the module NB_Store_CategoryFilter)
            If Not (Request.QueryString("catfilter") Is Nothing) Then
                If (Request.QueryString("andmode") Is Nothing) Then
                    _categoryList = Request.QueryString("catfilter").ToString()
                Else
                    If (Request.QueryString("catfilter").ToString() <> "") Then
                        ' reformat to AND select by adding "A" onto front, the SPROC will deal with it then
                        _categoryList = "A" + Request.QueryString("catfilter").ToString()
                    End If
                End If
            End If

            If _categoryList = "A" Then _categoryList = "" 'remove any possible catlist error

            If _CasCade And _categoryList = "" And CatID > 0 Then  'CategoryList overrides cascading search
                aryList = objCCtrl.GetCategories(PortalId, GetCurrentCulture)
                _categoryList &= objCCtrl.GetSubCategoryList(aryList, CatID)
                _categoryList &= CatID.ToString
            End If
            ...
            If FeaturedOnly Then
                If AllowCategoryBrowse = True AndAlso CatID > 0 Then
                    ...
                    aryList = objCtrl.GetProductList(PortalId, CatID, GetCurrentCulture, "", False, FeaturedOnly, _OrderByQuery, _OrderDESCQuery, ReturnLimit, _CurrentPage, PgSize, False, IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
                    ListSize = objCtrl.GetProductListSize(PortalId, CatID, GetCurrentCulture, "", False, FeaturedOnly, False, IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
                Else
                    'this should be featured always, nothing but... (CatID passed because featured default)
                    aryList = objCtrl.GetProductList(PortalId, assignCatDefault(), GetCurrentCulture, "", False, FeaturedOnly, _OrderByQuery, _OrderDESCQuery, ReturnLimit, _CurrentPage, PgSize, False, IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
                    ListSize = objCtrl.GetProductListSize(PortalId, assignCatDefault(), GetCurrentCulture, "", False, FeaturedOnly, False, IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
                End If
            Else
                If _WishList Then
                ...
                Else
                    If _CurrentPage < 2000 Then  'stop error when googole search silly pages.
                        aryList = objCtrl.GetProductList(PortalId, CatID, GetCurrentCulture, _searchQuery, False, FeaturedOnly, _OrderByQuery, _OrderDESCQuery, ReturnLimit, _CurrentPage, PgSize, GetStoreSettingBoolean(PortalId, "searchdescription.flag", GetCurrentCulture), IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
                        ListSize = objCtrl.GetProductListSize(PortalId, CatID, GetCurrentCulture, _searchQuery, False, FeaturedOnly, GetStoreSettingBoolean(PortalId, "searchdescription.flag", GetCurrentCulture), IsDealer(PortalId, UserInfo, GetCurrentCulture), _categoryList, ExcludeFeatured)
            ...
            If _categoryList <> "" Then
                If ctlPagingControl.QuerystringParams <> "" Then ctlPagingControl.QuerystringParams &= "&"
                ctlPagingControl.QuerystringParams &= "catfilter=" + _categoryList
            End If
                ...
                If (aryList.Count = 0 And CType(Settings("rblCategoryMessage"), String) = 3) Or CType(Settings("rblCategoryMessage"), String) = 2 Then
                    ...
                    If Not objCInfo Is Nothing Then
                        ...
                        If Not EventInterface.Instance() Is Nothing Then
                            Dim strMsg As String = EventInterface.Instance.getCategoryMessage(PortalId, CatID, UserInfo)
                            If strMsg <> "" Then
                                objCInfo.Message = strMsg
                            End If
                        End If
            ...
            If CatID > 0 Then
                ...
                If Not objCInfo Is Nothing Then
                    ...
                    If CType(Settings("chkBrowseCategory"), Boolean) = True Then
                        'only change if browseable list, so featured list, etc.. don't overwrite already applied title.
                        Me.BasePage.Title = CategoryPageName(objCInfo, PortalSettings.PortalName, Page.Title)
                    End If

                    If objCInfo.MetaKeywords <> "" Then
                        Me.BasePage.KeyWords = objCInfo.MetaKeywords
                    End If

                    If objCInfo.MetaDescription <> "" Then
                        Me.BasePage.Description = objCInfo.MetaDescription
                    Else
                        Me.BasePage.Description = objCInfo.CategoryDesc
                    End If

                    'include canonical link
                    If PortalSettings.HomeTabId = TabId Then
                        'include canonical link for home page.
                        IncludeCanonicalLink(Page, AddHTTP(PortalSettings.PortalAlias.HTTPAlias))
                    Else
                        If AllowCategoryBrowse Then
                            If objCInfo.SEOName <> "" Then
                                IncludeCanonicalLink(Page, GetProductListUrlByCatID(PortalId, TabId, CatID, objCInfo.SEOName, GetCurrentCulture))
                            Else
        ...
        Private Sub populateSP(ByVal ProdID As Integer)
            ...
            If aryList.Count = 1 Then
                ...
                If objPInfo.TagWords <> "" Then
                    Me.BasePage.KeyWords = objPInfo.SEOName & "," & objPInfo.TagWords
                End If

                If objPInfo.Summary <> "" Then
                    Me.BasePage.Description = objPInfo.SEOName & " " & objPInfo.Summary
                End If
        ...
        Private Function GetTemplProductCatID(ByVal ProdID As Integer) As Integer
            Dim aryList As ArrayList
            Dim objCtrl As New ProductController
            Dim objCCtrl As New CategoryController
            Dim objCInfo As New NB_Store_CategoriesInfo

            aryList = objCtrl.GetCategoriesAssigned(ProdID)
            For Each objPCInfo As NB_Store_ProductCategoryInfo In aryList
                objCInfo = objCCtrl.GetCategory(objPCInfo.CategoryID, GetCurrentCulture)
                If objCInfo.ProductTemplate <> "" Then
                    Return objCInfo.CategoryID
                End If
            Next
...
#End Region
        ...
        Private Sub ForceDocDownload(ByVal DocID As Integer)
            ...
            If Not objDInfo Is Nothing Then
                ...
                Response.TransmitFile(objDInfo.DocPath)
                Response.Flush()
                Response.End()
            End If
        ...
        Private Sub DoAfterBuyRedirect()
            ...
            If CType(Settings("chkRedirectToCart"), Boolean) Then
                ...
                Else
                    ...
                    End If
                End If
            ElseIf CType(Settings("chkNoRedirect"), Boolean) Then
                ' no redirect stay on current url
